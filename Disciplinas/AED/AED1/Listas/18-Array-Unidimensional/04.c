...
void percentual(float *notas, int qtAlunos) {
    int aprov = 0, repro = 0;
    float percentAp, percentRe;

    for(int c = 0; c < qtAlunos; c++) {
        //soma de alunos reprovados
        if(notas[c] < 6) repro++;
        //soma de alunos aprovados
        else aprov++;
    }

    percentAp = (float)aprov / qtAlunos * 100;
    percentRe = (float)repro / qtAlunos * 100;
    printf("Aprovados (%.2f%%)\nReprovados (%.2f%%)\n\n", percentAp, percentRe);
}
